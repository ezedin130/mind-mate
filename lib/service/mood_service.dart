...
class MoodService {
  ...
  Future<Map<String, dynamic>> submitMood(String userId, String mood) async {
  ...
  Future<Map<String, dynamic>> getMoodStreak(String userId) async {
    final response = await http.get(Uri.parse("$baseUrl/mood/streak/$userId"));
    return jsonDecode(response.body);
  }

  Future<Map<String, dynamic>> getAverageMood(String userId) async {
    final response = await http.get(Uri.parse("$baseUrl/mood/average/$userId"));
